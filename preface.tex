...
\paragraph{Materials} TheBounty supports \textit{multiple materials} for a single mesh. Materials are divided in different types(diffuse with specular reflection, transparency and translucency)

\smallskip

For more information about the different Material options, please refer to \tbrchapref{chapter:materials}.

\subparagraph{Diffuse} with specular reflection, transparency and translucency and support for shader nodes on various properties.
\subparagraph{Diffuse and glossy} material (Ashikhmin and Shirley), Blinn or anisotropic microfacet distribution with fresnel effect. Comes with shader nodes support on color, glossiness and bump.
\subparagraph{Coated} version of Diffuse and Glossy Diffuse, adding specular reflection with fresnel effect (dielectric).
\subparagraph{Glass}/dielectric material, with fresnel, filter, absorption and dispersion properties.
\subparagraph{Emit} material
\subparagraph{Blender Texture Layers} exported as shader nodes.
\subparagraph{Blend material} using a blend value or texture map.
\subparagraph{SSS} Experimental subsurface scattering support.
